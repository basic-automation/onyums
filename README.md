...
| **Live serve over Tor** (rendezvous circuit itself) | 🟡 | What remains live-only is arti's own part: descriptor publication, the rendezvous handshake, and the `DataStream` the handlers are handed. The `--ignored` live test is the only cover and CI does not run it. That test used to hang well past its own internal timeouts; it now runs on an explicit runtime with bounded teardown under a wall-clock watchdog that aborts and names the stuck phase — so it fails *loudly* rather than hanging — but it has not been run since, so treat this row as genuinely unverified rather than verified-elsewhere. |
