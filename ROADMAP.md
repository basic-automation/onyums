...
- [ ] Stable `ServiceProblem` projection on the handle — surface arti's `OnionServiceStatus::current_problem()` (the *reason* a service is Unreachable/Broken) as an onyums `#[non_exhaustive]`-proof enum, mirroring the `ServiceStatus` pattern. Needed because arti's `tor_hsservice::status::Problem` is itself `#[non_exhaustive]` and carries **no `Display`** (only `Debug`) in the pinned 0.43 source, so it can't be surfaced cleanly as-is. This is the concrete "why degraded/broken" signal the observability stream is missing.
